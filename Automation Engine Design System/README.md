# Automation Engine — Design System

> Internal operations console for a **Follow Up Boss** call-automation platform.
> This folder is the single source of truth for designing on-brand Automation Engine
...
**Automation Engine** is an internal admin console and **event-driven workflow platform** for
...
- **The set in use:** Filter, Apply (check), Reset/Replay (circular-arrow), Next/Prev (chevrons),
  ...
  `ui_kits/automation-engine/icons.jsx`, plus a few additions (Logout, Search, Refresh) drawn in the
  same language.
...
| `ui_kits/automation-engine/` | High-fidelity, interactive recreation of the console — see its own README. Start at `index.html`. |
...
- Lift components and copy patterns from `ui_kits/automation-engine/` rather than reinventing them.
- Obey the seven locked UX rules in §4. They're what makes a screen read as *Automation Engine*.
