# Handoff: Automation Engine — Console & Design System

## Overview
This bundle is the **Automation Engine design system + a high-fidelity console UI kit**. It defines
...
| `ui_kits/automation-engine/` | The interactive console. `index.html` runs it; `README.md` documents every screen + component file. |
...
## Screens
Each screen is documented in detail in **`ui_kits/automation-engine/README.md`** (purpose, layout,
...
`~/.claude/skills/automation-engine-design/` or a repo's `.claude/skills/`). `SKILL.md` lets Claude
Code load these guidelines on demand to design on-brand Automation Engine UI.

## How to run the reference
Open `ui_kits/automation-engine/index.html` in a browser (it loads React + Babel from CDN). Sign in
