...
- [ ] Run hosted smoke checks:
  ...
  - [ ] `GET /admin/webhooks`
  - [ ] `GET /admin/webhooks/stream` (SSE connects + heartbeat)
- [ ] Sync Follow Up Boss webhook URLs to hosted ingress:
  - [ ] Dry run: `./scripts/fub-webhook-sync.sh --dry-run`
  - [ ] Apply: `./scripts/fub-webhook-sync.sh`
  - [ ] Required env: `PUBLIC_BASE_URL`, `FUB_API_KEY`, `FUB_X_SYSTEM`, `FUB_X_SYSTEM_KEY`
