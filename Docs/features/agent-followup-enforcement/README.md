...
Entry point + phase tracker. Design rationale, FUB research, and field observations are in [`plan.md`](./plan.md). Canonical current workflow definition: [`workflow.json`](./workflow.json).
...
- `FubCreateNoteWorkflowStep` registered in the workflow step registry, taking config `{ mentionUserIds, mentionUserNames, message, subject? }`
  ...
  - **No `getUser` API call**; **no `FubUserDirectoryService`** — see [research.md](plan.md) "Why no `getUser` lookup"
