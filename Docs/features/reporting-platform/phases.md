...
| 1 — Dashboard metrics (direct slice) | Planned — [impl doc](./phase-1-implementation.md) (backend spec finalised 2026-06-19 rev 2) | Operational pipeline health, **all over one 24h window**: windowed run counts + success rate + deltas, hourly throughput, **operational** funnel `Ingested→Domain Events→Workflow Runs→Failed` (runs-only scope), view-only failures worklist, recent runs, events/min, and a **5-state health headline** (IDLE/ALL_CLEAR/HEALTHY/DEGRADED/UNHEALTHY). **One read-only snapshot endpoint + a dashboard-local JDBC read repo + a DTO — no framework.** |
