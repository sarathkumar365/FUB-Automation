...
Status: `CODE COMPLETE` (wrap-up log pending) — 3a (scaffold) + 3b (`fub_reassign` + A1–A7) + 3c (`fub_move_to_pond` + A1/A3/A4/A5) + 3d (`fub_add_tag` tracker-only append + C1–C3) + 3e (`fub_create_note` tracker-only **single-channel** + D1/D3/D4) all shipped. 682 tests green. See [phase-3a-implementation.md](./phase-3a-implementation.md) and [phase-3b-implementation.md](./phase-3b-implementation.md) for decision narratives; 3c reused 3b's scalar path verbatim; 3d is the first tracker-only append (FUB-first, record-on-success, no local write); 3e is the first entity-create wrap — reduced to a single `note.created` channel after the person-side echo was confirmed non-existent (FUB docs + empirical, 2026-06-01). Pending: `phase-3-implementation.md` wrap-up log.
